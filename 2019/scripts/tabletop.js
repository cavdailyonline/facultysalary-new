    ...
    */
        requestData: function (path, callback) {
            if (inNodeJS) {
                ...
                if (
                    supportsCORS &&
                    (!inLegacyIE || protocol === location.protocol)
                ) {
    ...
    */
        injectScript: function (path, callback) {
            ...
            if (this.singleton) {
                ...
                var self = this;
                callbackName =
                    "tt" + +new Date() + Math.floor(Math.random() * 100000);
        ...
        loadSheets: function (data) {
            ...
            for (i = 0, ilen = data.feed.entry.length; i < ilen; i++) {
                ...
                if (this.isWanted(data.feed.entry[i].content.$t)) {
                    ...
                    var sheet_id = data.feed.entry[i].link[linkIdx].href
                        .split("/")
                        .pop();
                    var json_path =
                        "/feeds/list/" +
                    ...
                    if (this.orderby) {
                        json_path +=
                            "&orderby=column:" + this.orderby.toLowerCase();
                    }
                    if (this.reverse) {
